...
Uwe Raabe, September 2019:
- removed SimpleParser.inc
- simplified VER defines using COMPILERVERSION
...
begin
  ...
  if not (TokenID in [ptIdentifier, ptString]) then
    NextNoJunk;
  while TokenID in [ptIdentifier, ptPoint, ptString, ptNull] do begin
    if TokenID = ptNull then
      Exit;
    Result := Result + Token;
    if StayAtIdentEnd and ((NextChar <> '.') and (not (IsIdentifiers(NextChar)))) then
      Exit;
    Next;
  end;
end;

function TmwBasePasLex.NextChar: Char;
begin
  Result := FBuffer.Buf[FBuffer.Run];
end;
